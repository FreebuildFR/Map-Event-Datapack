...
#execute as @e[x=4,y=13,z=-396,dx=113,dy=76,dz=113,type=minecart] at @s unless entity @e[type=minecraft:player,nbt={RootVehicle:{Entity:{id:"minecraft:minecart"}}},distance=..0.4] run kill @s
execute as @e[x=4,y=13,z=-396,dx=113,dy=76,dz=113,type=minecart] at @s unless predicate arc:has_passenger run kill @s

#Départ
execute as @a[x=3,y=86,z=-388,dx=0,dy=0,dz=0,nbt={RootVehicle:{Entity:{id:"minecraft:minecart"}}}] at @s run data merge entity @e[type=minecraft:minecart,distance=..0.5,limit=1] {Motion:[1.0,0.0]}
execute as @a[x=3,y=86,z=-388,dx=0,dy=0,dz=0,nbt={RootVehicle:{Entity:{id:"minecraft:minecart"}}}] run item replace entity @s weapon.mainhand with bow{display:{Name:'{"text":"Arc"}'},HideFlags:5,Unbreakable:1b,Enchantments:[{id:"minecraft:infinity",lvl:1s}]}
execute as @a[x=3,y=86,z=-388,dx=0,dy=0,dz=0,nbt={RootVehicle:{Entity:{id:"minecraft:minecart"}}}] run item replace entity @s weapon.offhand with minecraft:arrow{display:{Name:'{"text":"Fléchettes"}'}} 1
